  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-background px-4 py-8">
      <div className="w-full max-w-md rounded-3xl border border-border bg-card p-8 shadow-xl">
        <h1 className="text-2xl font-bold text-foreground">Restricted Admin Signup</h1>
        <p className="mt-1 text-sm text-muted-foreground">Only approved domains or a valid admin secret key can register.</p>
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-4">
          ...
          <div>
            ...
            <p className="mt-1 text-xs text-muted-foreground">Must include uppercase, lowercase, number, and symbol.</p>
          </div>

          <div>
            <label className="mb-1.5 block text-xs font-medium text-muted-foreground uppercase tracking-wide">Admin Secret Key (optional)</label>
            <div className="relative">
              <KeyRound className="absolute left-3.5 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <input
                type="password"
                value={form.adminSecretKey}
                onChange={(e) => setForm((f) => ({ ...f, adminSecretKey: e.target.value }))}
                className="w-full rounded-xl border border-input bg-background py-3 pl-10 pr-4 text-sm text-foreground outline-none focus:border-cyan-500/40 focus:ring-1 focus:ring-cyan-500/20"
              />
            </div>
          </div>
        ...
        <p className="mt-6 text-center text-sm text-muted-foreground">
          Already have admin access? <Link href="/login" className="text-cyan-400 hover:text-cyan-300">Sign in</Link>
        </p>
      </div>
    </div>
  );
